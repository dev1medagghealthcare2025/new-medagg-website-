...
const MobileHeroSlide1 = ({
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: showIndependenceDayTheme
          ? 'url(\'/Mobile-view-independence.png\')'
          : 'url(\'/Herosection background.jpg\')',
      }}
    >
      {/* Overlay - Shown only when Independence Day period is over */}
      ...
      <div className={`absolute left-4 top-4 z-20 flex items-center gap-3 ${showIndependenceDayTheme ? 'text-[#1a1446]' : 'text-white'}`}>
      ...
      <div className='relative z-10 px-4 pt-[76px] text-white'>
        ...
        {showIndependenceDayTheme && (
          <div className='inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-full px-3 py-1 shadow-sm mb-2'>
            <svg className='w-4.5 h-3 rounded-sm shadow-[0_1px_3px_rgba(0,0,0,0.15)]' viewBox='0 0 3 2'>
            ...
            <span className='text-[10px] font-extrabold tracking-wide flex gap-1 select-none'>
              <span className='text-[#FF9933]'>Happy</span>
              <span className='text-[#128807]'>Independence Day</span>
            </span>
          </div>
        )}

        {showIndependenceDayTheme ? (
          <div className='inline-block bg-white/85 backdrop-blur-sm border border-gray-200/50 rounded-2xl px-4 py-3.5 shadow-sm mt-1 mb-2.5'>
            <h1 className='text-[22px] font-extrabold leading-tight'>
              <span className='block text-[#ff7300]'>Freedom to Live.</span>
              <span className='block mt-0.5 text-[#128807]'>Freedom from Pain.</span>
            </h1>
            <div className='w-20 h-0.5 bg-gradient-to-r from-[#FF9933] via-gray-200 to-[#128807] rounded-full mt-2.5' />
          </div>
        ) : (
          <h1 className='text-[22px] font-extrabold leading-tight text-white'>
            <span className='block'>No Surgery. No Scars.</span>
            <span className='block mt-0.5'>Just Results.</span>
          </h1>
        )}
      ...
      <div className='absolute left-4 right-4 top-[193px] z-30'>
        <SharedSearchBar
          ...
          compact
          theme={showIndependenceDayTheme ? 'light' : 'dark'}
        />
      </div>

      <div className='absolute bottom-[248px] left-48 z-20 text-left'>
        <div className={`mb-2.5 text-[13px] font-bold ${showIndependenceDayTheme ? 'text-[#1a1446]/80' : 'text-white'}`}>Follow Us On :</div>
        <div className='flex items-center justify-start gap-3.5 pointer-events-auto'>
          <a href='https://www.facebook.com/profile.php?id=61558841344582' target='_blank' rel='noopener noreferrer' aria-label='Facebook'
            className={showIndependenceDayTheme ? 'inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#1a1446]/20 text-[#1a1446] hover:bg-[#1a1446]/10 transition-all' : 'inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/45 text-white hover:bg-white/20 transition-all'}>
