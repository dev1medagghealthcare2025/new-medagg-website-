...
const TACE_Herosection = ({ city = '', variant = '' }) => {
  ...
  return (
    <section
      ...
      style={{
        backgroundImage: `linear-gradient(rgba(45,37,82,0.35), rgba(45,37,82,0.35)), url('${backgroundImage}')`,
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-center'>
          <div className='text-center lg:text-left'>
            <h1 className='text-3xl sm:text-4xl lg:text-5xl font-extrabold leading-tight text-white'>
              {isCitySpecific ? (
                <>TACE Treatment for Liver Tumors in {cityName}</>
              ) : (
                <><span className='text-white'>TACE (Transarterial </span><br /><span className='text-white'>Chemoembolization)</span></>
              )}
            </h1>
            <p className='mt-4 text-sm sm:text-base lg:text-lg text-white/85 max-w-2xl mx-auto lg:mx-0'>
              {isCitySpecific
                ? 'Advanced non-surgical liver tumor treatment by Interventional Radiology specialists | NoSurgeries by Medagg'
