...
const PAEWhySurgeries = ({ city = '', variant = '' }) => {
          ...
          <div>
            ...
            {isCitySpecific ? (
              <>
                ...
                <h2 className='text-3xl sm:text-4xl font-extrabold text-[#ff3576] leading-tight'>
                  {isDelhi
                    ? 'In Delhi, No-Surgery Treatment for Enlarged Prostate is Available'
                    : `In ${isChennai ? 'Chennai' : isMadurai ? 'Madurai' : isCoimbatore ? 'Coimbatore' : isBangalore ? 'Bangalore' : 'Mangalore'}, No-Surgery Option is Available for Enlarged Prostate`}
                </h2>
              </>
            ) : (
            ...
            <div className='mt-6 rounded-xl border border-[#ff3576]/30 bg-white px-5 py-4'>
              <p className='text-gray-700'>
                <span className='font-semibold text-[#ff3576]'>Enlarged Prostate(BPH):</span> No surgeries for enlarged prostate using Prostate Artery Embolization(PAE) to improve urinary symptoms and quality of life.
              </p>
            </div>
...
export default PAEWhySurgeries;
